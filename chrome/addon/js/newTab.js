...
window.onload=function(){
    ...
    const date = new Date();
    brandTitle.innerHTML = `Cursors ${date.getFullYear()}`
    ...
    chrome.topSites.get(topSites=>{
        ...
        topSites.forEach(site=>{
          ...
          chrome.storage.local.get([faviconUrl],(ob)=>{
            ...
            // if website logo breaks => put default icon
            logo.onerror = () => {
              const fallBackIcon = chrome.runtime.getURL("images/icon.png");
              logo.src = fallBackIcon;
            }
        })
